...
jQuery(function($) {
    wcPaoInitAddonTotals = {
        ...
        init: function(cart) {
            ...
            $(cart).filter('.iwptp-product-form .cart, .iwptp-add-to-cart-wrapper .cart').find('.wc-pao-addon-checkbox-group-required').each(function() {
            ...
            .on('woocommerce-product-addons-update', function() {
                ...
                if (qty) {
                    ...
                    if (formatted_sub_total && '1' == $totals.data('show-sub-total')) {
                        var productName = $(this).closest('.iwptp-product-form').find('.iwptp-product-form-title').clone().find('.iwptp-product-form-price').remove().end().text(),
